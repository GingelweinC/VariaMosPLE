...
function upsertProp(list: any[], name: string, value: any) {
  ...
  else { list.push(new Property(name, "string", value)); }
}
...
function createElement(
  ...
  const el: any = {
    ...
    properties: (properties ?? []).map(p => new Property(p.name, "string", p.value))
  };
    ...
    switch (op.op) {
      ...
      case "connect": {
        ...
        const rel = ps.createRelationship(
          ...
          op.min ?? 0,
          op.max ?? 1,
          (op.properties ?? []).map(p => new Property(p.name, "string", p.value))
        );
